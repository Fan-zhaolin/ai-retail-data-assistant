...
class EvaluateTests(unittest.TestCase):
    ...
    def test_unsafe_sql_is_recorded_without_execution(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            questions.write_text(
                "id,question,type,expected_block\n"
                "1,delete the sales table,security,true\n",
                encoding="utf-8-sig",
            )
    ...
    def test_limit_only_evaluates_first_n_questions(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            evaluate_questions(
                ...
                run_sql=lambda _sql: pd.DataFrame({"value": ["ok"]}),
                build_prompt=lambda q: q,
                limit=1,
                verbose=False,
            )
    ...
    def test_ids_only_evaluates_selected_question(self):
        from src.evaluate import evaluate_questions
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            evaluate_questions(
                ...
                results_path=results,
                bad_cases_path=bad_cases,
                ask_llm=lambda _prompt: "SELECT 1 AS value",
                run_sql=lambda _sql: pd.DataFrame({"value": [1]}),
                build_prompt=lambda question: question,
                ids={"G02"},
                verbose=False,
            )

            with results.open(encoding="utf-8-sig") as file:
                rows = list(csv.DictReader(file))
            self.assertEqual([row["id"] for row in rows], ["G02"])
